...
macOS SwiftUI app (Swift 6.0, macOS 15+) built with Swift Package Manager. Generates professional invoices (factures) and quotes (devis) as PDF, with multi-currency support (EUR, USD, USDC, USDT, BTC, ETH) and blockchain payment tracking.
...
- **PDF via CoreText/CoreGraphics** — `PDFGenerator` draws directly to `CGContext`. No SwiftUI ImageRenderer. Coordinate system is top-down (y=0 = top of page), converted to CG bottom-up via `cgY()`. Text rendered with `CTLineCreateWithAttributedString` + `CTLineDraw`.
...
- `Models/` — Document, LineItem, ClientInfo, CompanyInfo, Currency, Blockchain, Enums, TransactionSignature. All are Codable.
- `Services/DataStore.swift` — JSON persistence, CRUD operations
...
- `Views/Documents/DocumentEditorView.swift` — main editor with all sections (header, dates, currency, client, lines, totals, payment, signatures, notes)
...
Release Please on push to `main`. Conventional Commits required (`feat:`, `fix:`, etc.). On release merge: builds .app + .dmg on macOS 15 runner, uploads to GitHub Release.
