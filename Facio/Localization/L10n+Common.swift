...
extension L10n {
    ...
    static func facturXOnlyEUR(_ l: AppLanguage, currency: String) -> String { l == .fr ? "Factur-X n'est disponible qu'en EUR pour le moment : indisponible en \(currency). Passez la facture en EUR." : "Factur-X is currently only available in EUR: unavailable in \(currency). Switch the invoice to EUR." }
    ...
    static func facturXUnsupportedExemptLine(_ l: AppLanguage) -> String { l == .fr ? "Une ligne à 0 % sur une facture avec TVA n'est pas encore prise en charge pour la facture électronique : l'exonération ne peut pas être qualifiée automatiquement. Retirez la ligne à 0 % ou appliquez-lui un taux de TVA." : "A 0% line on an invoice that charges VAT is not yet supported for e-invoicing: the exemption cannot be qualified automatically. Remove the 0% line or apply a VAT rate to it." }
    static func facturXMissingSellerTaxRegistration(_ l: AppLanguage) -> String { l == .fr ? "Renseignez votre SIRET (Paramètres › Entreprise) : il est obligatoire pour une facture en franchise de TVA (art. 293 B)." : "Add your SIRET (Settings › Company): it is required for a VAT-exempt (franchise) invoice." }
    static func facturXGenerationFailed(_ l: AppLanguage) -> String { l == .fr ? "Impossible de générer la facture électronique." : "Could not generate the e-invoice." }
